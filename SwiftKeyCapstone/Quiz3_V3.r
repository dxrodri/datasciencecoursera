...
if(doStemming) {
  ...
  corpusData.stemmed <- tm_map(corpusData, stemDocument,language="english")
...
if(doStemCompletion) {
    corpusTDMUnigrams.stem <- stemCompletion(rownames(corpusTDMUnigrams), dictionary=corpusData.dict, type=c("prevalent"))
    # change to stem completed row names
    rownames(corpusTDMUnigrams) <- as.vector(corpusTDMUnigrams.stem )
}
...
if(useBigramForPrediction) {
  ...
  #corpusTDMBigrams.stemmed <- TermDocumentMatrix(corpusData, control = list(stemming=TRUE, wordLengths = c(minWordLength, maxWordLength), tokenize = BigramTokenizer))
  corpusTDMBigrams <- removeSparseTerms(corpusTDMBigrams,sparsePercentage)
  #corpusTDMBigrams.stemmed.small <- removeSparseTerms(corpusTDMBigrams.stemmed,sparsePercentage)
  freqBigram <- rowSums(as.matrix(corpusTDMBigrams))
  #freqBigram <- sort(rowSums(as.matrix(corpusTDMBigrams)), decreasing=TRUE)
  #corpusSmallDict <- PlainTextDocument( names(freqBigram))
  if(doStemCompletion) {
    corpusTDMBigrams.stem <- stemCompletion(rownames(corpusTDMBigrams), dictionary=corpusData.dict, type=c("prevalent"))
    #corpusTDMBigrams.stemmed.small.complete <- stemCompletion(rownames(corpusTDMBigrams.stemmed.small), dictionary=corpusData.dict, type=c("prevalent"))
    # change to stem completed row names
    rownames(corpusTDMBigrams) <- as.vector(corpusTDMBigrams.stem )
  }
  #corpusDataFinal <- tm_map(corpusData.stemmed, stemCompletion, dictionary=corpusSmallDict, type=c("first"))
...
http://statistical-research.com/text-mining/

k = 2;
02.SEED = 1234;
03.my_TM =
04.list(VEM = LDA(myDtm, k = k, control = list(seed = SEED)),
05.VEM_fixed = LDA(myDtm, k = k,
06.control = list(estimate.alpha = FALSE, seed = SEED)),
07.Gibbs = LDA(myDtm, k = k, method = "Gibbs",
08.control = list(seed = SEED, burnin = 1000,
09.thin = 100, iter = 1000)),
10.CTM = CTM(myDtm, k = k,
11.control = list(seed = SEED,
12.var = list(tol = 10^-4), em = list(tol = 10^-3))));
13. 
14.Topic = topics(my_TM[["VEM"]], 1);
15. 
16.#top 5 terms for each topic in LDA
17.Terms = terms(my_TM[["VEM"]], 5);
18.Terms;
19. 
20.(my_topics =
21.topics(my_TM[["VEM"]]));
22. 
23.most_frequent = which.max(tabulate(my_topics));
24. 
25.terms(my_TM[["VEM"]], 10)[, most_frequent];


generateSentences <- function(corpus) {
  sentence_token_annotator <- Maxent_Sent_Token_Annotator()
  corpus <- as.String(corpus)
  sentence.boundaries <- annotate(corpus, sentence_token_annotator)
  
  corpus[sentence.boundaries]
}
