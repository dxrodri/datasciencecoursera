...
baltimoreEmissionsData = NEI_dt[NEI_dt$fips == "24510", list(totalEmissions = sum(Emissions)), by = c("type","year")]
png('plot3.png')
p3 = ggplot(data=baltimoreEmissionsData, aes(x=year, y=totalEmissions, colour=type)) 
p3 = p3 +  geom_line(aes(group=type)) +  geom_point(size=3)
p3 = p3 + labs(title="plot3 - Baltimore total emissions by type")
p3
dev.off()
#colnames(NEI)
