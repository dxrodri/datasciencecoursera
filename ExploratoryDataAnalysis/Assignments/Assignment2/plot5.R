...
baltimoreEmissionsData = NEI_dt[NEI_dt$fips == "24510", ]

joinedData = join(baltimoreEmissionsData, motorVehiclesGroup, by=c("SCC"),type="inner")

joinedEmissionsData = joinedData[, list(totalEmissions = sum(Emissions)), by = c("EI.Sector","year")]
png('plot5.png')
p5 = ggplot(data=joinedEmissionsData, aes(x=year, y=totalEmissions, colour=EI.Sector)) 
p5 = p5 +  geom_line(aes(group=EI.Sector)) +  geom_point(size=3)
p5 = p5 + labs(title="plot5 -  vehicle total emissions by EI.Sector in Baltimore, Maryland")
p5 = p5 + scale_fill_continuous(guide = guide_legend()) + theme(legend.direction = "vertical", legend.position="bottom")
p5
dev.off()
#colnames(NEI)
